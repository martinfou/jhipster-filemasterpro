...
type RestOf<T extends IProject | NewProject> = Omit<T, 'createdDate' | 'lastUpdatedDate'> & {
...
export class ProjectService {
  ...
  protected convertDateFromClient<T extends IProject | NewProject | PartialUpdateProject>(project: T): RestOf<T> {
    return {
      ...
      lastUpdatedDate: project.lastUpdatedDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restProject: RestProject): IProject {
    return {
      ...
      lastUpdatedDate: restProject.lastUpdatedDate ? dayjs(restProject.lastUpdatedDate) : undefined,
    };
  }
